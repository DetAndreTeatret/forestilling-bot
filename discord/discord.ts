...
import {getLinkedDiscordUser} from "../database/coolUser.js"
...
import {consumeFoodButton, createAndSendFoodOrderEmbed} from "./food.js"
...
export class SuperClient extends Client { // TODO look over methods inside/outside SuperClient
    ...
    async createNewChannelForEvent(guild: Guild, event: Event, dayTime: boolean, logger: Logger) {
        ...
        await createAndSendFoodOrderEmbed(channel)
...
export async function startDiscordClient() {
    ...
    client.on(Events.InteractionCreate, async function (interaction) {
        if (interaction.isChatInputCommand()) {
            ...
            try {
                const interactionTyped = interaction as ButtonInteraction

                // Outsource the handling, if any...
                await consumeFoodButton(interactionTyped)
            } catch (error) {
                console.error(error)
                if (interaction.replied || interaction.deferred) {
                    await interaction.followUp({
                    ...
                    await interaction.reply({ // TODO abstract out error handling
                        content: "There was an error while executing this command!",
                        ephemeral: true
                    })
                }
...
export function findPinnedEmbedMessage(message: PinnedEmbedMessages, pinnedMessages: Collection<string, Message<true>>) {
 ...
 */
export enum PinnedEmbedMessages {
    FOOD_ORDER,
    CAST_LIST,
    EVENT_STATUS
}
