...
import {addNewUser, fetchUser} from "../../database/coolUser.js"
...
export const data = new SlashCommandBuilder()
    .setName("linkuser")
    .setDescription("Link a Discord id and a SchedgeUp user")
    .addStringOption(option => option.setName("schedgeup-id").setDescription("Schedgeup user name").setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    const schedgeUpUser = users.find(u => u.displayName.toLowerCase().trim() === schedgeUpId.toLowerCase().trim())
    if (schedgeUpUser) {
        await addNewUser(schedgeUpUser.userId, discordUser.id, schedgeUpUser.displayName)
