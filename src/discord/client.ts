...
export async function startDiscordClient() {
    ...
    client.on(Events.InteractionCreate, async function (interaction) {
        ...
        if (interaction.channel && interaction.isButton()) {
            ...
            if (path[1] !== "button") console.warn("Faulty custom ID in button click " + path[1])
            switch (path[0]) {
                case "food": {
        ...
        if (interaction.isModalSubmit()) {
            const path = interaction.customId.split("-")
            if (path[1] !== "modal") console.warn("Faulty custom ID in modal submit " + path[1])
            switch (path[0]) {
                case "announcement": {
        ...
        if (interaction.isAnySelectMenu()) {
            const path = interaction.customId.split("-")
            if (path[1] !== "picker") console.warn("Faulty custom ID in select menu " + path[1])
            switch (path[0]) {
                case "announcement": {
